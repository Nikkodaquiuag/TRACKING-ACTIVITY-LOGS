...
function insertNewUser($pdo, $username, $first_name, $last_name, $email, $gender, $address, $password) {
	$response = array();
	$checkIfUserExists = checkIfUserExists($pdo, $username); 

	if (!$checkIfUserExists['result']) {

		$sql = "INSERT INTO user_accounts (username, first_name, last_name, email, gender, address, password) 
		VALUES (?,?,?,?,?,?,?)";

		$stmt = $pdo->prepare($sql);

		if ($stmt->execute([$username, $first_name, $last_name, $email, $gender, $address, $password])) {
			$response = array(
				"status" => "200",
